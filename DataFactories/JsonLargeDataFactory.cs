...
public class JsonLargeDataFactory : ITestDataFactory<FreightDboTblLoads>
{
    public FreightDboTblLoads CreateRecord()
    {
        var now = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();
        var guid = Guid.NewGuid().ToString();

        return new FreightDboTblLoads
        {
            Id_ = 100001,
            LoadDate = now,
            SystemDate = now,
            ...
            CustomerBillTotal = 4500.0000m,
            Weight = 42000,
            ChargeCustomerUnloadingTotal = 150.0000m,
            PayCarrierUnloadingTotal = 100.0000m,
            CurrentCarrierStatus = 4,
            VoidReason = null,
            DeliveredLoadChange = false,
            InternalDatSearch = 0,
            DriverReload = false,
            MinPayTruck = 2000.0000m,
            MaxPayTruck = 3000.0000m,
            DateTurnedIn = now,
